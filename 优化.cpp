	...
	int paraIndex)
{
	...
	double obj1 = func(input, output, para1, objIndex);// obj = A * sin(Bx) + C * cos(D*x) - F
...
void gaussNewton(const VectorXd& input, const VectorXd& output, VectorXd& params)
{
	...
	{
		obj = objF(input, output, params);//每次优化之后计算100个误差 x1 * sin(x2 * t) + x3 * cos(x4 * t) - f;F（x）

		double sum = 0;
		sum = Func(obj);//F = (f ^t * f)/2
